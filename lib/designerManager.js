...
exports.checkSpaceUserBeforeUpdate = async function checkSpaceUserBeforeUpdate(spaceId, userId, companyId) {
    ...
    if (companyId) { // 校验是否是单位管理员
        let permissionSet = (await steedosSchema.getObject('permission_set').find({ filters: `(space eq '${spaceId}') and (name eq 'workflow_admin')` }))[0];
        if (!permissionSet || !permissionSet.users || !permissionSet.users.includes(userId)) {
            throw new Error('该用户无删除流程权限');
        }
        if (companyId != spaceUser.company_id) {
            throw new Error('不能删除非本分部下的流程');
        }
