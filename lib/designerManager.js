...
exports.getByAdminSpaceIds = async function getByAdminSpaceIds(spaceIds, companyId, isCompanyAdmin) {
    ...
    let users = await new Promise(function (resolve, reject) {
        Fiber(function () {
            try {
                let result = Creator.getCollection('users').find({ _id: { $in: userIds } }, { fields: { 'photo': 1, 'google_id': 1, 'imo_uid': 1, 'company': 1, 'name': 1, 'locale': 1, 'steedos_id': 1, 'primary_email_verified': 1, 'is_paid_user': 1, 'mobile': 1, 'email': 1, 'created': 1, 'modified': 1, 'created_by': 1, 'modified_by': 1, 'email_notification': 1, 'qq_open_id': 1 } }).fetch();
                resolve(result);
            } catch (error) {
                reject(error)
            }
    ...
    await new Promise(function (resolve, reject) {
        Fiber(function () {
            try {
                ...
                if (ff.app === 'workflow') {
                    let insCount = Creator.getCollection('instances').find({ space: spaceId, form: formId, 'form_version': form['current']['id'] }).count();
                    if (insCount > 0) {
                        pass = true;
                    }
                } else if (ff.app === 'creator') {
                    let recordsCount = Creator.getCollection('records').find({ space: spaceId, form: formId, 'form_version': form['current']['id'] }).count();
                    if (recordsCount > 0) {
                        pass = true;
                    }
                }
...
exports.getSpaceUser = async function getSpaceUser(userId, spaceId) {
    let spaceUser = (await steedosSchema.getObject('space_users').find({ filters: `(space eq '${spaceId}') and (user eq '${userId}')` }))[0];
    if (!spaceUser) {
